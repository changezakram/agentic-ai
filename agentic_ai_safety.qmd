...
Safety training alone doesn’t prevent this. Even heavily aligned models can be manipulated through multi-turn conversations, role-playing scenarios, and encoded instructions.^9^ Multi-modal models are especially vulnerable—instructions embedded in images can bypass text-based safeguards entirely.^9^
...
This risk is not theoretical. Research from UC Berkeley demonstrates that poisoned retrieval databases can hijack agent behavior without retraining the model.^10^ The malicious content can persist unnoticed until activation.
