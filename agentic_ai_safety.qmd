...
The common thread isn't the mechanism—SQL, Python, retrieval, or prompts. It's an architectural shift. Traditional software executes what developers explicitly code. Agentic systems execute what they're convinced is the correct next action. This makes persuading the model as effective as exploiting technical vulnerabilities. Attackers don't need buffer overflows or memory corruption—they just need convincing language.

The threat model expands to include natural language as an attack surface, data as executable instruction, reasoning as a controllable process, and tools as amplification mechanisms.
...
**Confidentiality** extends beyond customer and enterprise data. It must also protect system prompts, internal instructions, API credentials, orchestration logic, and sometimes the model itself. A single leaked instruction or credential can redirect the entire system's behavior.

**Integrity** includes more than business data accuracy. It must cover the model supply chain, training data, embeddings, retrieval pipelines, and tool execution paths. If any part of this chain is compromised, the agent's reasoning becomes corrupted.

**Availability** means more than web service uptime. In agentic systems, it includes sustained model performance, inference stability, retrieval responsiveness, and tool execution reliability. An agent that times out, degrades, or fails mid-task can cause cascading operational failures.
...
The protection requirements multiply—not just the consequences of failure.

---

### Why the Attack Surface Expands

LLMs in production systems expand the attack surface. Traditional software only executes what developers explicitly program. Agents must process external inputs, ingest real-time data, and dynamically decide what actions to take.

This creates new risk categories. Sensitive information can leak through model outputs. Prompts and tool instructions can be manipulated by external content. Models and embedding pipelines can be corrupted through supply-chain attacks. Tool invocations can be hijacked to perform malicious operations.

These attacks don't target a single vulnerability. They exploit the interaction between components—language, retrieval, reasoning, and action—inside a tightly coupled decision loop.

Agentic AI security requires more than traditional application security or conventional ML security. It's a different security problem.

---

## Evaluation and Risk Assessment — Why Model Testing Is No Longer Enough

Many AI risk programs make a critical mistake: they evaluate the language model and assume they've evaluated the system. This assumption fails completely in agentic AI.
...
Risk emerges from component interaction, not individual components. Security evaluation must shift from model-centric testing to end-to-end adversarial system evaluation.
...
AgentXploit tests agents under strict, realistic constraints: the attacker cannot modify the user's request and cannot observe or alter the agent's internal mechanics. The user query is assumed to be benign. The agent's code, prompts, and orchestration logic are inaccessible.
...
The attack surface becomes the data ecosystem surrounding the agent, not the agent itself.

---

### How Environment-Based Attacks Are Discovered

AgentXploit uses a fuzzing-based adversarial search framework driven by Monte Carlo Tree Search. Instead of mutating input prompts, it mutates elements of the agent's environment—web content, files, documents, and retrieved context.

Each mutation is evaluated with a simple binary signal: did the agent perform a prohibited action, or not?

This success/failure signal feeds back into the search process, allowing the system to iteratively evolve more effective attacks. Over time, the framework discovers exploits that emerge only through multi-step reasoning inside the agent's planning loop.
...
Consider a shopping assistant agent that helps users find products across retail sites. A user asks the agent to find a high-quality screen protector for their phone.
An attacker doesn't alter this request. Instead, they post a malicious product review on a legitimate e-commerce page with a hidden instruction: visit an external website controlled by the attacker.
