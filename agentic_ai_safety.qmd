...
**Safety training doesn't prevent this.** Research shows that even heavily aligned models get manipulated through multi-turn conversations, role-playing scenarios, and encoded instructions.^9^ Multi-modal models are easier to attack—adversarial instructions embedded in images bypass text-based safeguards entirely.
...
This risk is not theoretical. Research from UC Berkeley demonstrates that poisoned retrieval databases can hijack agent behavior without retraining the model.^9^ The malicious content can remain dormant until it is triggered.
...
[9] Wang, B., et al. (2023). DecodingTrust: A Comprehensive Assessment of Trustworthiness in GPT Models.
